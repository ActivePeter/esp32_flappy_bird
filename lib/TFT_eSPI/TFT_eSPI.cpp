  ...
  if ((size==1) && fillbg) {
    ...
    for (int8_t i = 0; i < 6; i++ ) {
      ...
      if (size == 1) { // default size
        ...
        for (int8_t j = 0; j < 8; j++) {
          if (line & 0x1) {
            //drawRectToBuffer(x + (i * size), y + (j * size), size, size, color);
...
//字体修改核心执行位置
  ...
  // Font is not 2 and hence is RLE encoded
  {
    ...
    if ((textsize != 1) || (textcolor == textbgcolor)) {
      drawRectToBuffer((135-width * textsize-x),pY,width * textsize,textsize * height,0);
        ...
        if (line & 0x80) {
          ...
          while (line--) { // In this case the while(line--) is faster
            ...
            if (ts) {
              tnp = np;
              while (tnp--) {
                //buffer[px*240+py]=textcolor;
              //tft_Write_16(textcolor);
              }
            }
            else {
              buffer[(135-px)*240+py]=textcolor;
              //tft_Write_16(textcolor);
            }
            px += textsize;
